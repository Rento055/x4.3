...
function p5(v)
    edit(0x2df524, v, "にゃんこチケット", true);
    gg.toast("にゃんチケ成功");
end

function p7(v)
    edit(0x2df52c, v, "レアチケット", true);
    gg.toast("レア成功");
end
...
function p11(v)
    gg.clearResults();
    gg.searchNumber("-257~~256", 4, false, 2^29, base +0x3a9f0, base + 0x3b63c);
...
function p12()
    local err = {156,183,286,321,340,354,433,434,466,493,498,499,500,501,674,741,742,743,744,745,746,782,783,784,785,786,787};
    local editval = gg.getValues({{address = base +0x3b63c, flags = 4}})[1].value;
    for _, n in ipairs(err) do
        gg.setValues({{
            address = base +0x3a9ec +n *0x4, 
            flags = 4, 
            value = editval
        }});
    end
