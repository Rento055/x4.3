...
function main()
    ...
    for i = 1, #menu-1 do
        xpcall(function()
            if mp[i] == true then _ENV["p"..i](mp[i]);end
...
function p5(v)
    edit(0x2dad50, v, "にゃんこチケット", true);
    gg.toast("にゃんチケ成功");
end

function p7(v)
    edit(0x2dad58, v, "レアチケット", true);
    gg.toast("レア成功");
end
...
function p11(v)
    gg.clearResults();
    gg.searchNumber("-257~~256", 4, false, 2^29, base +0x3a9f0, base + 243236);
...
function p12()
    local err = {156, 183, 286, 321, 340, 354, 433, 434, 466, 493, 498, 499, 500, 501, 674, 741, 742, 743, 744, 745, 746};
    local editval = gg.getValues({{address = base +243236, flags = 4}})[1].value;
    for _, n in ipairs(err) do
        gg.setValues({{
            address = base + 243236 -782*0x4 +n*0x4, 
            flags = 4, 
            value = editval
        }});
    end
